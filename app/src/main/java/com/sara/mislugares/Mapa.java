    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        while (c.moveToNext()) {
            ...
            if (p != null && p.getLatitud() != 0) {
                ...
                Bitmap iGrande = iconoDrawable.getBitmap();
                Bitmap icono = Bitmap.createScaledBitmap(iGrande, iGrande.getWidth() / 7, iGrande.getHeight() / 7, false);
                mapa.addMarker(new MarkerOptions().position(new LatLng(p.getLatitud(), p.getLongitud())).title(c.getString(1)).snippet(c.getString(2)).icon(BitmapDescriptorFactory.fromBitmap(icono)));
            }
        }
        mapa.setOnInfoWindowClickListener(this);
    }
